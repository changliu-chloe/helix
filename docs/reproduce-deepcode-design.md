# 设计文档：参考 DeepCode 的实验复现 / 自主实验工作流

> 本文档覆盖 helix「复现」板块中**从论文理解或实验假设，到代码实现、远程运行、结果回流**的 agent 工作流设计。
...
短期不新增过多顶层 skill，`reproduce` 主 skill 只保留总入口、阶段路由和安全门禁；阶段细节放到
`skills/reproduce/references/`，进入某一阶段时再按需读取，避免主 `SKILL.md` 变成过大的操作手册。

`type:repro`（复现论文）走论文事实链路：
...
| 阶段 | 适用类型 | 详细协议 |
|---|---|---|
| `paper-to-setup` | `repro` | `skills/reproduce/references/paper-to-setup.md` |
| `setup-to-plan` | `repro` | `skills/reproduce/references/setup-to-plan.md` |
| `hypothesis-to-plan` | `mine` | `skills/reproduce/references/hypothesis-to-plan.md` |
| `plan-to-code` | `repro` / `mine` | `skills/reproduce/references/plan-to-code.md` |
| `run-monitor-analyze` | `repro` / `mine` | `skills/reproduce/references/run-monitor-analyze.md` |
| `result-to-claim` | `mine` | `skills/reproduce/references/result-to-claim.md` |
...
### 3.5 `run-monitor-analyze`：运行、监控、结果回流
...
### 3.6 `result-to-claim`：自主实验结果到 claim
...
| `reproduce` | 主入口，按阶段路由到 `references/*.md` 并维护工作区进度 |
...
| `result-to-claim` / `experiment-audit` | 实验完成后判断证据能支持什么结论 |
...
- 精简 `skills/reproduce/SKILL.md`：主文件只保留阶段路由、阶段 gate 和安全规则。
- 新增 `skills/reproduce/references/*.md`：按阶段渐进式披露 `repro` / `mine` 两类流程的详细协议。
