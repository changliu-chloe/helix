...
> **数据布局**：所有本地数据落在单一 `workspace/` 下——`workspace/notes/{papers,daily,reviews}`、
> `workspace/experiments/`、`workspace/.helix/`（索引）。gitignore 只忽略 `workspace/` 一处即可。
> `notes_dir` 填绝对路径可指向外部 Obsidian vault（则留在外部）。旧版数据散在顶层的，`git pull` 后跑
> `helix migrate --yes` 搬进 workspace/（只搬不删、先校验）。
