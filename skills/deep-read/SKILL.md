...
- 用户给了标题：先 `arxo search "<标题关键词>" --top-n 5` 找到对应论文，取其 `paper_id`

### 2. 生成笔记骨架
```bash
arxo note new <arxiv_id>
```
...
### 5. 自动链接 + 建索引
```bash
arxo note link <笔记路径>    # 把正文里出现的其他论文名自动变成 wikilink
arxo index build             # 重建 FTS 索引，让这篇能被本地检索到
```
