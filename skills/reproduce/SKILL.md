---
name: reproduce
description: 当用户想在自己的机器上复现某篇论文的实验（给了 arXiv id/标题/已有笔记），需要总结原文实验设置、判断本机 GPU 装不装得下、并产出可执行复现方案时使用。
---

# 论文复现规划

把一篇论文的实验「翻译」成能在用户自己 GPU 上跑的复现方案。arxo 负责确定性活
（显存估算/判级/建工作区骨架），**实验设置的理解、方案的填充由你（agent）完成**。

方法论借鉴 ref/deepcode 的 Paper2Code（先抽取算法与实验设置，再产出分段式复现计划），
但只做到「可执行复现方案」层，不自动写实现代码。

> **运行约定**：所有 arxo 命令用 `uv run arxo ...` 在项目根执行。若 `uv` 不可用，回退 `python -m arxo.cli ...`。
...
### 1. 建复现工作区骨架
```bash
uv run arxo repro new <笔记路径 或 arXiv id> [--domain "<方向>"] [--draft]
```
- 传**已有笔记路径**最好（自动读 frontmatter 拿标题/方向/回链）；也可传 arXiv id。
- 在 `repro/<方向>/<短名>/` 生成 `setup.md`（实验设置）+ `plan.md`（复现计划）骨架。
- **`--draft`**：落到 `draft_notes/` 而非 `repro/`——需求测试、试跑方案时用这个。
- 输出 JSON 含 `workspace` 路径；骨架里每个 `<!-- agent: … -->` 就是你要填的。
...
```bash
uv run arxo repro vram --params <B> --dtype <精度> --ctx <长度> --batch <N>
...
- **测试用 --draft**：需求测试先落 `draft_notes/`，稳定了再进 `repro/`。
